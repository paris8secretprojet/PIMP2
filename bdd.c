...
int nb_row_file(char * char_file, int n){
  //Permet de connaitre le nombre de ligne d un fichier.
...
int nb_pixel_file(char * char_file){
  //Permet de connaitre le nombre de pixel d une data d un fichier.
...
void load_data_image(data_image * im, int n, int * pix){
  //A partir des donnees des fichiers "DATA" on remplit une
  //structure "data_image" en normalisant les donnees entre -1 et 1.
  int i;
...
int nb_char_file(FILE * fd){
  ////Permet de connaitre le "char" d un fichier.
...
void extract_char_file(FILE * fd, int n, char * char_file){
  //Extraction des "char" d un fichier vers un vecteur de "char".
