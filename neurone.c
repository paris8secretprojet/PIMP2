...
void prediction(main_network * main_ntw){
  ...
  for(i=0 ; i<main_ntw->test->n ; i++){
	...
	printf("\tOUTPUT %d = %f\n\n",j,main_ntw->ntw[j].lyr[0].outputs->mat[i][0]);
	printf("SCORE 1 = %f\n",score[i]);
  ...
  for(i=0 ; i<main_ntw->test->n ; i++){
    printf("EXCEPTED : %d --->\t\t PREDICT : %d\n",(int)main_ntw->current_batch_class->mat[i][0],predi[i]);
    if((int)main_ntw->current_batch_class->mat[i][0]==predi[i]){
      accuracy++;
    }
  }
...
//void build_image(image * picture, char * nom){
...
int main(){
  ...
  */
  ...
  image picture0, picture1, picture2, picture3, picture4, picture5, picture6, picture7, picture8, picture9;
  image * pic0, * pic1, * pic2, * pic3, * pic4, * pic5, * pic6, * pic7, * pic8, * pic9;
  pic0=&picture0, pic1=&picture1, pic2=&picture2, pic3=&picture3, pic4=&picture4, pic5=&picture5, pic6=&picture6, pic7=&picture7, pic8=&picture8, pic9=&picture9;

  image carac0, carac1, carac2, carac3, carac4, carac5, carac6, carac7, carac8, carac9;
  image * c0, * c1, * c2, * c3, * c4, * c5, * c6, * c7, * c8, * c9;
  c0=&carac0, c1=&carac1, c2=&carac2, c3=&carac3, c4=&carac4, c5=&carac5, c6=&carac6, c7=&carac7, c8=&carac8, c9=&carac9;
  ...
  */
  ...
  pic0=lireExtraire(pic0,"0.ppm");
  c0=loadChar(pic0,c0,delimit(pic0));
  formatted=resize(c0,formatted);
  ...
  pic1=lireExtraire(pic1,"1.ppm");
  c1=loadChar(pic1,c1,delimit(pic1));
  formatted=resize(c1,formatted);
  conversion(main_ntw,formatted);
  ...
  pic2=lireExtraire(pic2,"2.ppm");
  c2=loadChar(pic2,c2,delimit(pic2));
  formatted=resize(c2,formatted);
  conversion(main_ntw,formatted);
  
  
  printf("\n\n\t3\n\n");
  
  pic3=lireExtraire(pic3,"3.ppm");
  c3=loadChar(pic3,c3,delimit(pic3));
  formatted=resize(c3,formatted);
  conversion(main_ntw,formatted);


  printf("\n\n\t4\n\n");

  pic4=lireExtraire(pic4,"4.ppm");
  c4=loadChar(pic4,c4,delimit(pic4));
  formatted=resize(c4,formatted);
  conversion(main_ntw,formatted);

  printf("\n\n\t5\n\n");
  

  pic5=lireExtraire(pic5,"5.ppm");
  c5=loadChar(pic5,c5,delimit(pic5));
  formatted=resize(c5,formatted);
  conversion(main_ntw,formatted);
  ...
  pic6=lireExtraire(pic6,"6.ppm");
  c6=loadChar(pic6,c6,delimit(pic6));
  formatted=resize(c6,formatted);
  conversion(main_ntw,formatted);
  ...
  pic7=lireExtraire(pic7,"7.ppm");
  c7=loadChar(pic7,c7,delimit(pic7));
  formatted=resize(c7,formatted);
  conversion(main_ntw,formatted);
  ...
  pic8=lireExtraire(pic8,"8.ppm");
  c8=loadChar(pic8,c8,delimit(pic8));
  formatted=resize(c8,formatted);
  conversion(main_ntw,formatted);

  printf("\n\n\t9\n\n");
  
  pic9=lireExtraire(pic9,"9.ppm");
  c9=loadChar(pic9,c9,delimit(pic9));
  formatted=resize(c9,formatted);
  conversion(main_ntw,formatted);
  
  

  
  dupliquer(formatted,"copie3_1.ppm");
