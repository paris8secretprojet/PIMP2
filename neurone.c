...
void prediction(main_network * main_ntw){
  ...
  for(i=0 ; i<10 ; i++){
    ...
    for(j=0 ; j<main_ntw->test->n ; j++){
      ...
      printf("\tOUTPUT %d = %f\n",j,main_ntw->ntw[i].lyr[(main_ntw->ntw[i].nb_layer)-1].outputs->mat[j][0]);
...
int main(){
  ...
  pic=lireExtraire(pic,"1.ppm");
  c=loadChar(pic,c,delimit(pic));
  formatted=resize(c,formatted);
  conversion(main_ntw,formatted);

  pic=lireExtraire(pic,"2.ppm");
  c=loadChar(pic,c,delimit(pic));
  formatted=resize(c,formatted);
  conversion(main_ntw,formatted);

  pic=lireExtraire(pic,"3.ppm");
  c=loadChar(pic,c,delimit(pic));
  formatted=resize(c,formatted);
  conversion(main_ntw,formatted);

  pic=lireExtraire(pic,"4.ppm");
  c=loadChar(pic,c,delimit(pic));
  formatted=resize(c,formatted);
  conversion(main_ntw,formatted);

  pic=lireExtraire(pic,"5.ppm");
  c=loadChar(pic,c,delimit(pic));
  formatted=resize(c,formatted);
  conversion(main_ntw,formatted);

  pic=lireExtraire(pic,"6.ppm");
  c=loadChar(pic,c,delimit(pic));
  formatted=resize(c,formatted);
  conversion(main_ntw,formatted);

  pic=lireExtraire(pic,"7.ppm");
  c=loadChar(pic,c,delimit(pic));
  formatted=resize(c,formatted);
  conversion(main_ntw,formatted);
  
  pic=lireExtraire(pic,"8.ppm");
  c=loadChar(pic,c,delimit(pic));
  formatted=resize(c,formatted);
  conversion(main_ntw,formatted);
  
  pic=lireExtraire(pic,"9.ppm");
  c=loadChar(pic,c,delimit(pic));
  formatted=resize(c,formatted);
  conversion(main_ntw,formatted);
  ...
  */
  ...
  dupliquer(formatted,"copie9_3.ppm");
